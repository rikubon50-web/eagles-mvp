...
export default function ContactPage() {
  ...
  const validate = () => {
    ...
    if (!message.trim()) next.message = "お問い合わせ内容は必須です";
    setErrors(next);
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 md:px-8">
      <header className="pt-8 md:pt-12">
        <h1 className="section-title text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900">Contact</h1>
        ...
        <form onSubmit={onSubmitToConfirm} className="mt-8 md:mt-12 space-y-8">
          ...
          <div>
            ...
            <textarea
              ...
              rows={10}
              className={`w-full resize-y rounded-md border bg-slate-50 px-4 py-4 text-lg outline-none transition
                ${errors.message ? "border-red-500" : "border-slate-300 focus:border-[#0f6536]"}
              `}
        ...
        <form onSubmit={onSubmitFinal} className="mt-8 md:mt-12 space-y-6">
          <div className="rounded-lg border border-slate-300 bg-white">
            ...
            <div className="p-4 space-y-4">
              <div>
                <div className="text-xs font-semibold text-slate-500">お名前</div>
                <div className="mt-1 text-lg">{name}</div>
              </div>
              <div className="border-t pt-4">
                <div className="text-xs font-semibold text-slate-500">お電話番号</div>
              ...
              <div className="border-t pt-4">
                <div className="text-xs font-semibold text-slate-500">お問い合わせ内容</div>
                <p className="mt-1 whitespace-pre-wrap text-lg leading-7">{message}</p>
              </div>
            </div>
          </div>
