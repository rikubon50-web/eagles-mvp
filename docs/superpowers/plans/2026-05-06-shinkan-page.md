# 新歓特設ページ Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `/shinkan` — a recruitment landing page for university freshmen, using the existing `news` API (filtered by category=新歓) and `players` API for testimonials. No new microCMS API needed.

**Architecture:** Server component page with ISR. Static content (stats, steps, FAQ) is hardcoded. Dynamic content (新歓ニュース, 先輩の声) is fetched at build/revalidation time from existing microCMS endpoints. Full-width section breakout technique (same as homepage). FadeIn scroll animations (existing component).

**Tech Stack:** Next.js 14 App Router, Tailwind CSS, Framer Motion FadeIn (existing), microCMS `news` + `players` endpoints (existing client).
...
|---|---|---|
| Modify | `src/lib/microcms.ts` | Add `fetchNewsByCategory()` function |
| Create | `src/app/shinkan/page.tsx` | Full 新歓ページ (7 sections) |
| Modify | `src/components/Header.tsx` | Add 新歓 nav link in desktop + mobile nav |

---

### Task 1: Add fetchNewsByCategory to microcms.ts

**Files:**
- Modify: `src/lib/microcms.ts`

Current file ends at line 191. Add this function after `fetchNewsById`:

- [ ] **Step 1: Add the function**

Open `src/lib/microcms.ts` and append after the `fetchNewsById` function (after line ~139):
...
export default async function ShinkanPage() {
  ...
  const [shinkanNews, players] = await Promise.all([
    fetchNewsByCategory("新歓", 6).catch(() => []),
    fetchPlayers().catch(() => []),
  ]);
...
```bash
pnpm build 2>&1 | grep -E "error|Error|shinkan|✓"
```

Expected: `○ /shinkan` appears in the route table and `✓ Compiled successfully`.

- [ ] **Step 3: Commit**

```bash
git add "src/app/shinkan/page.tsx" src/lib/microcms.ts
git commit -m "feat: 新歓特設ページ追加 (/shinkan)"
```

---

### Task 3: Add 新歓 link to Header

**Files:**
- Modify: `src/components/Header.tsx`

Current desktop nav (lines 29–37):
```tsx
...
Current mobile nav (lines 62–69):
```tsx
...
- [ ] **Step 1: Add 新歓 to desktop nav — insert after BLOG, before ROSTER**

Replace the desktop nav block with:
```tsx
<nav className="header__nav">
  <Link href="/" className="header__link">TOP</Link>
  <Link href="/about" className="header__link">ABOUT</Link>
...
- [ ] **Step 2: Add 新歓 to mobile nav — same position**

Replace the mobile nav block with:
```tsx
<nav className="header__nav--mobile">
  <Link href="/" className="header__link" onClick={() => setOpen(false)}>TOP</Link>
  <Link href="/about" className="header__link" onClick={() => setOpen(false)}>ABOUT</Link>
...
git commit -m "feat: ヘッダーナビに新歓リンク追加"
```
